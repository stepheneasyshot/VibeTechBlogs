...
![blogs_network_login_cookie](/images/blog/blogs_network_login_cookie.webp)

作用：一般都是客户端给自己作标记，服务器拿本地的 ```Cookie``` 来对比。HTTP是无状态的，使用这个机制可以让服务器标识不同的用户。例如会话管理，登录状态，购物车，用户偏好，分析用户行为。

用户tracking流程图：

![blogs_network_tracking](/images/blog/blogs_network_tracking.webp)
